...
# Connecting to DB
conn = connector.connect(host='localhost', user='root', password='2020BTECS00033', database='std') 
c = conn.cursor()
...
usr_name = Label(window ,text = f"Connected to DB as: 2020BTECS00033", background="orange").grid(row = 0, column = 1, pady=20)
    ...
    try:
        ...
        if id is not None:
            ...
            if len(c.fetchall()) == 0:
                ...
                Label(newWindow ,text = f"Update values in table: {selected_tb}", background="orange").grid(row = 0, column = 0, padx=10, pady=10)
                c.execute(f'''show columns
                              FROM {selected_tb}
                        ''')
